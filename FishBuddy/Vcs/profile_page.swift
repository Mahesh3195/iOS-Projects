...
@available(iOS 12.0, *)
class profile_page: UIViewController,CollectionDelegate {
    ...
    func initialization(){
        ...
        navv.Nav_View_with_back_btn(_title: "Profile", back_page: "log_catch_pg", back_obj: sign_in_page())
    ...
    func design_initialization(){
        ...
        let edit_btn = Buttons.small_btn(x: ui.get_w(o: imv) - (Stored.width/4) - 15, y: ui.get_h(o: imv) - 45, _title: "Edit Profile", title_Clr: Colors.white, bg_clr: Colors.orange, sbv: scrl_view)
        ...
        let followers_view = Views.cust_view(_x: 15, y: ui.add_y_pos(o: desc_lbl, s: 15), w: ui.get_w(o: scrl_view)-30, _h: 130, bgcolor: Colors.Clear_color(), _sbv: scrl_view)
        
        let followers_lbl = Lables.Avenir_Book_med_lbl_14(x: 0, y: 0, _wid: ui.get_w(o: followers_view)/2, hei: 20, title: "Followers", title_clr: Colors.orange, _sv: followers_view)
        
        let followers_btn = Buttons.right_arw_btn(_x: ui.get_w(o: followers_view)-130, _y: 0, _wid: 130, title: "See All Followers", sbv: followers_view)
        followers_btn.addTarget(self, action: #selector(followers_btn_clicked(_:)), for: .touchUpInside)
        
        followers_cltn = Custom_Collectionview();
        followers_cltn.delegate = self
        followers_cltn._cell = "followers_cltn_cell"
        
        if(Stored.Device == "iPad"){
            followers_lbl.frame.size.height = 30
            followers_btn.frame.size.width = 210
            followers_btn.frame.origin.x = ui.get_w(o: followers_view)-210
            followers_cltn.set_layout(top_val: 0, left_val: 0, bottom_val: 0, right_val: 0, item_width: 170, item_height: 170, min_lineSpace: 5, min_IntrSpace: 0, scr_dirction: .horizontal)
            //followers_cltn._coll.frame.origin.y = 35
            followers_view.frame.size.height = 220
            //followers_view.backgroundColor = UIColor.yellow
        }else{
            followers_cltn.set_layout(top_val: 0, left_val: 0, bottom_val: 0, right_val: 0, item_width: 80, item_height: 100, min_lineSpace: 5, min_IntrSpace: 0, scr_dirction: .horizontal)
        }
        
        followers_cltn.setup(_x: 0, _y: 20, _width: ui.get_w(o: followers_view), _height: 100, _bgClr: Colors.Clear_color(), _tag: 201)
        followers_cltn._coll.showsHorizontalScrollIndicator = false

        followers_cltn.data = imgs_arr;
        followers_cltn._coll.reloadData();
        
        followers_view.addSubview(followers_cltn._coll)
        
        if(Stored.Device == "iPad"){
            followers_cltn._coll.frame.origin.y = 30
            followers_cltn._coll.frame.size.height = 170
        }
        
        let following_view = Views.cust_view(_x: 15, y: ui.add_y_pos(o: followers_view, s: 0), w: ui.get_w(o: scrl_view)-30, _h: 130, bgcolor: Colors.Clear_color(), _sbv: scrl_view)
        
        let following_lbl = Lables.Avenir_Book_med_lbl_14(x: 0, y: 0, _wid: ui.get_w(o: following_view)/2, hei: 20, title: "Following", title_clr: Colors.orange, _sv: following_view)
        
        let following_btn = Buttons.right_arw_btn(_x: ui.get_w(o: following_view)-75, _y: 0, _wid: 75, title: "See All", sbv: following_view)
        following_btn.addTarget(self, action: #selector(following_btn_clicked(_:)), for: .touchUpInside)
        
        following_cltn = Custom_Collectionview();
        following_cltn.delegate = self
        following_cltn._cell = "catches_cltn_cell"
        
        if(Stored.Device == "iPad"){
            following_lbl.frame.size.height = 30
            following_lbl.textAlignment = .left
            following_btn.frame.size.width = 110
            following_btn.frame.origin.x = ui.get_w(o: following_view)-110
            following_cltn.set_layout(top_val: 0, left_val: 0, bottom_val: 0, right_val: 0, item_width: 170, item_height: 170, min_lineSpace: 5, min_IntrSpace: 0, scr_dirction: .horizontal)
            //followers_cltn._coll.frame.origin.y = 35
            following_view.frame.size.height = 220
            //following_view.backgroundColor = UIColor.yellow
        }else{
            following_cltn.set_layout(top_val: 0, left_val: 0, bottom_val: 0, right_val: 0, item_width: 80, item_height: 100, min_lineSpace: 5, min_IntrSpace: 0, scr_dirction: .horizontal)
        }
        
        
        following_cltn.setup(_x: 0, _y: 20, _width: ui.get_w(o: following_view), _height: 100, _bgClr: Colors.Clear_color(), _tag: 202)
        following_cltn._coll.showsHorizontalScrollIndicator = false

        following_cltn.data = imgs_arr;
        following_cltn._coll.reloadData();
        
        following_view.addSubview(following_cltn._coll)
        
        if(Stored.Device == "iPad"){
            following_cltn._coll.frame.origin.y = 30
            following_cltn._coll.frame.size.height = 170
        }
        
        
        let catches_view = Views.cust_view(_x: 15, y: ui.add_y_pos(o: following_view, s: 5), w: ui.get_w(o: scrl_view)-30, _h: 130, bgcolor: Colors.Clear_color(), _sbv: scrl_view)
        ...
        catches_btn.addTarget(self, action: #selector(see_all_btn_clicked(_:)), for: .touchUpInside)
        ...
        catches_cltn.setup(_x: 0, _y: 20, _width: ui.get_w(o: catches_view), _height: 100, _bgClr: Colors.Clear_color(), _tag: 203)
        ...
        scrl_view.contentSize = CGSize(width: 0, height: ui.get_h(o: imv) + ui.get_h(o: followers_view) + ui.get_h(o: following_view) + ui.get_h(o: catches_view) + ui.get_h(o: catches_map_view) + 150)
        
    }
    
    @objc func followers_btn_clicked(_ sender: UIButton){
        Stored.btn_clicked = "Followers"
        Stored.val = "Follow"
        navv.Navigation_redirecting_page(_Pagename: "followers_pg", _nav: sideMenuController() as! SideMenuNavigationController)
    }
    
    @objc func following_btn_clicked(_ sender: UIButton){
        Stored.btn_clicked = "Following"
    ...
    func collection_didSelect(index: Int, collectionview: UICollectionView) {
        ...
        if(collectionview.tag == 201){
            Stored.cell_clicked = "Followers"
            navv.Navigation_redirecting_page(_Pagename: "followers_inner_pg", _nav: sideMenuController() as! SideMenuNavigationController)
        }else if(collectionview.tag == 202){
            Stored.cell_clicked = "Following"
            navv.Navigation_redirecting_page(_Pagename: "followers_inner_pg", _nav: sideMenuController() as! SideMenuNavigationController)
        }else{
    ...
    @objc func see_all_btn_clicked(_ sender: UIButton){
